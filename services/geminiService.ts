...
// Split the VITE_GEMINI_API_KEY environment variable by commas to support multiple keys
const apiKeys = import.meta.env.VITE_GEMINI_API_KEY.split(',')
    .map(key => key.trim())
    .filter(key => key);

if (apiKeys.length === 0) {
    throw new Error("VITE_GEMINI_API_KEY environment variable is set, but contains no valid keys.");
}
